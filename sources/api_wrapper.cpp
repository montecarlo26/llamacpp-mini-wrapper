...
int language_model::initialize() {
    ...
    params.seed = 12345678;//fixed seed to get the same output every time
...
int language_model::send_input_and_receive_output(std::string input_text, std::string& output_text) {
    ...
    //variables to set with each pass of input text
    ...
    while (n_remain != 0 && !is_antiprompt) {
        ...
        if (input_echo && display) {
            for (auto id : embd) {
                ...
                output_text.append(token_str.c_str());
...
}//end send input and receive output
...
int language_model::save_inputs_and_outputs() {
